...
def test_and_visualize():
    """
    测试修正后的适中策略V7（周K）并可视化买卖点
    """
    print("=" * 100)
    print("修正后的适中布林带策略V7（周K版本）- 回测与可视化")
    print("=" * 100)
    print()
    
    # 创建输出文件夹
    output_dir = 'results/moderate_boll_v7_weekly'
    ...
    strategy = ModerateBollStrategyV7(
        ...
        sell_threshold=2.5   # 卖出阈值：价格高过中线的150%（价格 > 中线 × 2.5）
    )
...
def analyze_sell_signals(symbol, signals_df):
    ...
    for idx, row in sell_signals.iterrows():
        ...
        # 判断是否高过布林带中线的150%
        is_above_threshold = current_price >= middle_band * 2.5
        
        if is_uptrend:
            uptrend_sells += 1
            print(f"  {idx.strftime('%Y-%m-%d')}: 上行期卖出 (价格: {current_price:.2f}, 中线: {middle_band:.2f}, 高过中线: {((current_price / middle_band) - 1) * 100:.2f}%)")
        else:
            downtrend_sells += 1
...
def visualize_trades(symbol, data, trades, output_dir):
    ...
    fig.suptitle(f'{symbol} - 修正后适中布林带策略V7（周K）交易信号', fontsize=16, fontweight='bold')
    ...
    # 绘制卖出阈值线（中线的2.5倍）
    sell_threshold_line = data['middle_band'] * 2.5
    ax1.plot(data.index, sell_threshold_line, label='卖出阈值（中线×2.5）', linewidth=1.5, alpha=0.6, color='purple', linestyle='-.')
...
def visualize_detailed_trades(symbol, data, trades, output_dir):
    ...
    for i in range(num_trades):
        ...
        # 绘制卖出阈值线（中线的2.5倍）
        sell_threshold_line = plot_data['middle_band'] * 2.5
        ax.plot(plot_data.index, sell_threshold_line, label='卖出阈值（中线×2.5）', linewidth=1.5, alpha=0.6, color='purple', linestyle='-.')
