...
class ModerateBollStrategyV8(BaseStrategy):
    ...
    def __init__(self, period=20, std_dev=2, min_uptrend_days=20, min_interval_days=10, 
        ...
        Args:
            ...
            trailing_stop_pct: 移动止损比例（默认0.10即10%）
        """
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # 移动止损状态
        highest_price = 0  # 持仓期间的最高价格
        highest_price_idx = -1  # 最高价格的日期索引
        ...
        for i in range(len(df)):
            ...
            if position == 1:
                # 检查是否在上行期
                if in_uptrend:
                    # 移动止损：价格从高点回落超过trailing_stop_pct
