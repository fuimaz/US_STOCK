...
2. 买入时机是在上行期，出现股价跌破boll下轨，且间隔2周以上的1次反弹都没有上穿boll带中轨，第二次或者之后的反弹穿过中轨后，回落到中轨之下买入
...
class ModerateBollStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(df)):
            ...
            # 4. 买入信号：在上行期，出现股价跌破boll下轨，且间隔2周以上的1次反弹都没有上穿boll带中轨，
            # 第二次或者之后的反弹穿过中轨后，回落到中轨之下买入
            if in_uptrend and position == 0:
                # 检查是否有足够的破下轨次数
                if len(lower_band_breaks) >= 2:
                    # 检查是否有足够的穿中轨次数（改为2次即可）
