...
def test_daily_boll_strategy():
    """
    测试日线布林带策略
    """
    print("=" * 80)
    print("日线布林带策略测试（5年数据）")
    print("=" * 80)
    ...
    # 测试股票列表（从缓存中提取）
    symbols = [
        'AAPL', 'ABBV', 'AMT', 'AMZN', 'APD', 'BAC', 'CAT', 'CCI', 'CHTR', 'CMCSA',
        'COP', 'CVX', 'DD', 'DOW', 'DUK', 'D', 'EQIX', 'EXC', 'FCX', 'GE',
        'GOOGL', 'GS', 'HAL', 'HON', 'JNJ', 'JPM', 'KO', 'LIN', 'MCD',
        'META', 'MMM', 'MRK', 'MSFT', 'MS', 'NEE', 'NKE', 'O', 'PFE', 'PG',
        'PLD', 'RTX', 'SLB', 'SO', 'TMUS', 'TSLA', 'T', 'UNH', 'VZ',
        'WFC', 'WMT', 'XOM'
    ]
    period = '5y'
    ...
    for symbol in symbols:
        ...
        try:
            ...
            #     print(f"✓ K线图已保存到: daily_boll_{symbol}_kline.png")
    ...
    print("=" * 80)
    print()
    print(f"{'股票代码':<10} {'状态':<10} {'策略总收益':<12} {'原始涨幅':<12} {'超额收益':<12} {'策略年化':<12} {'原始年化':<12} {'超额年化':<12}")
    print("-" * 100)
    
    for result in results_summary:
        print(f"{result['symbol']:<10} {result['status']:<10} {result['total_return']:<12.2f}% {result['buy_hold_return']:<12.2f}% {result['excess_return']:<12.2f}% {result['annualized_return']:<12.2f}% {result['buy_hold_annual']:<12.2f}% {result['excess_annual']:<12.2f}%")
    ...
    if len(successful_stocks) > 0:
        ...
        print(f"✓ 表现最好的股票: {best_stock['symbol']}（策略总收益: {best_stock['total_return']:.2f}%）")
        ...
        print()
        print("=" * 80)
        print(f"正在绘制表现最好的股票 {best_stock['symbol']} 的K线图...")
        print("=" * 80)
        ...
        try:
            ...
            plot_kline_with_signals(best_stock_signals, best_stock['symbol'], output_dir)
...
def plot_kline_with_signals(data: pd.DataFrame, symbol: str, output_dir: str = None):
    """
    绘制K线图并标记买卖点
    ...
    fig, axes = mpf.plot(
        ...
        type='candle',
        style=s,
        title=f'{symbol} Daily Bollinger Strategy - Buy/Sell Signals',
        ylabel='Price ($)',
        volume=True,
        addplot=apds,
        figsize=(16, 10),
    ...
    if output_dir:
        ...
        output_file = f'daily_boll_{symbol}_kline.png'
