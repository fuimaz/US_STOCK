"""
测试修正后的适中布林带策略V8（周K版本，使用移动止损）并可视化
"""
import pandas as pd
import numpy as np
from data_fetcher import DataFetcher
from moderate_boll_strategy_v8 import ModerateBollStrategyV8
...
def test_and_visualize():
    """
    测试修正后的适中策略V8（周K，移动止损）并可视化买卖点
    """
    print("=" * 100)
    print("修正后的适中布林带策略V8（周K版本，使用移动止损）- 回测与可视化")
    print("=" * 100)
    print()
    
    # 创建输出文件夹
    output_dir = 'results/moderate_boll_v8_weekly'
    ...
    # 初始化策略（调整参数适应周K）
    strategy = ModerateBollStrategyV8(
        ...
        trailing_stop_pct=0.10  # 移动止损比例：10%
    )
...
def analyze_sell_signals(symbol, signals_df):
    ...
    for idx, row in sell_signals.iterrows():
        ...
        if is_uptrend:
            uptrend_sells += 1
            print(f"  {idx.strftime('%Y-%m-%d')}: 上行期卖出（移动止损） (价格: {current_price:.2f}, 中线: {middle_band:.2f})")
        else:
            downtrend_sells += 1
    ...
    print(f"  上行期卖出（移动止损）: {uptrend_sells}")
...
def visualize_trades(symbol, data, trades, output_dir):
    ...
    fig.suptitle(f'{symbol} - 修正后适中布林带策略V8（周K，移动止损）交易信号', fontsize=16, fontweight='bold')
