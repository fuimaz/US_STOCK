"""
测试修正后的适中布林带策略V6（周K版本）并可视化
"""
import pandas as pd
import numpy as np
from data_fetcher import DataFetcher
from moderate_boll_strategy_v6 import ModerateBollStrategyV6
...
def test_and_visualize():
    """
    测试修正后的适中策略V6（周K）并可视化买卖点
    """
    print("=" * 100)
    print("修正后的适中布林带策略V6（周K版本）- 回测与可视化")
    print("=" * 100)
    print()
    
    # 创建输出文件夹
    output_dir = 'results/moderate_boll_v6_weekly'
    ...
    # 初始化策略（调整参数适应周K）
    strategy = ModerateBollStrategyV6(
        period=20,           # 周K的20周期约等于日K的100周期（5个月）
        std_dev=2,
        min_uptrend_days=20,  # 周K的20周约等于日K的100天（5个月）
        min_interval_days=10, # 周K的10周约等于日K的50天（2.5个月）
        ma_period=60,        # 周K的60周约等于日K的300天（15个月）
        uptrend_threshold=0.5,
        max_hold_days=120,   # 最大持仓120天（约6个月）
        sell_threshold=1.5   # 卖出阈值：价格超过中线的50%
    )
    ...
    for symbol in test_symbols:
        ...
        try:
            ...
            # 转换为周K数据
            data = resample_to_weekly(daily_data)
            
            print(f"✓ 数据转换完成")
            print(f"  日K数据量: {len(daily_data)} 条")
            print(f"  周K数据量: {len(data)} 条")
            ...
            # 分析卖出信号
            analyze_sell_signals(symbol, signals_df)
...
def analyze_sell_signals(symbol, signals_df):
    """
    分析卖出信号的市场状态
    """
    print()
    ...
    for idx, row in sell_signals.iterrows():
        ...
        # 判断是否超过布林带中线的50%
        is_above_threshold = current_price >= middle_band * 1.5
        
        if is_uptrend:
            uptrend_sells += 1
            print(f"  {idx.strftime('%Y-%m-%d')}: 上行期卖出 (价格: {current_price:.2f}, 中线: {middle_band:.2f}, 超过中线: {((current_price / middle_band) - 1) * 100:.2f}%)")
        else:
            downtrend_sells += 1
            print(f"  {idx.strftime('%Y-%m-%d')}: 下行期卖出（止损） (价格: {current_price:.2f}, 中线: {middle_band:.2f})")
    
    print()
    print(f"  总卖出信号数: {len(sell_signals)}")
    print(f"  上行期卖出: {uptrend_sells}")
    print(f"  下行期卖出（止损）: {downtrend_sells}")
    print()
...
def visualize_trades(symbol, data, trades, output_dir):
    ...
    fig.suptitle(f'{symbol} - 修正后适中布林带策略V6（周K）交易信号', fontsize=16, fontweight='bold')
    ...
    # 绘制卖出阈值线（中线的1.5倍）
    sell_threshold_line = data['middle_band'] * 1.5
    ax1.plot(data.index, sell_threshold_line, label='卖出阈值（中线×1.5）', linewidth=1.5, alpha=0.6, color='purple', linestyle='-.')
    ...
    if not buy_signals.empty:
        ax1.scatter(buy_signals.index, buy_signals['Close'], color='red', marker='^', s=300, 
                   label='买入', zorder=5, edgecolors='black', linewidths=2)
    
    if not sell_signals.empty:
        ax1.scatter(sell_signals.index, sell_signals['Close'], color='green', marker='v', s=300, 
                   label='卖出', zorder=5, edgecolors='black', linewidths=2)
    
    ax1.set_ylabel('价格', fontsize=12)
    ax1.set_title('价格走势与交易信号（周K）', fontsize=14)
    ax1.legend(loc='upper left', fontsize=10)
    ax1.grid(True, alpha=0.3)
    
    # 绘制布林带宽度
    bb_width = data['upper_band'] - data['lower_band']
    ax2.plot(data.index, bb_width, label='布林带宽度', linewidth=1.5, color='purple')
...
def visualize_detailed_trades(symbol, data, trades, output_dir):
    ...
    fig.suptitle(f'{symbol} - 详细交易分析（周K）', fontsize=16, fontweight='bold')
    ...
    for i in range(num_trades):
        ...
        end_idx = min(len(data) - 1, sell_idx + 20)
        ...
        # 绘制价格和布林带
        ax.plot(plot_data.index, plot_data['Close'], label='收盘价', linewidth=2, alpha=0.8)
        ax.plot(plot_data.index, plot_data['upper_band'], label='上轨', linewidth=1.5, alpha=0.6, color='red', linestyle='--')
        ax.plot(plot_data.index, plot_data['middle_band'], label='中轨', linewidth=1.5, alpha=0.6, color='orange', linestyle='--')
        ax.plot(plot_data.index, plot_data['lower_band'], label='下轨', linewidth=1.5, alpha=0.6, color='green', linestyle='--')
        
        # 绘制卖出阈值线（中线的1.5倍）
        sell_threshold_line = plot_data['middle_band'] * 1.5
        ax.plot(plot_data.index, sell_threshold_line, label='卖出阈值（中线×1.5）', linewidth=1.5, alpha=0.6, color='purple', linestyle='-.')
        
        # 绘制买入卖出点
        ax.scatter([buy_date], [buy_price], color='red', marker='^', s=400, 
                  label='买入', zorder=5, edgecolors='black', linewidths=2)
        ax.scatter([sell_date], [sell_price], color='green', marker='v', s=400, 
                  label='卖出', zorder=5, edgecolors='black', linewidths=2)
        ...
        info_text += f'持仓周数: {(sell_date - buy_date).days // 7} 周\n'
