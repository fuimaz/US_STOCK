"""
测试调整后的适中布林带策略V4（正确版本）在周K上的表现并可视化
"""
import pandas as pd
import numpy as np
...
def test_and_visualize():
    """
    测试调整后的适中策略V4在周K上并可视化买卖点
    """
    print("=" * 100)
    print("调整后的适中布林带策略V4（正确版本）- 周K回测与可视化")
    print("=" * 100)
    print()
    
    # 创建输出文件夹
    output_dir = 'results/moderate_boll_v4_weekly'
...
def visualize_trades(symbol, data, trades, output_dir):
    ...
    fig.suptitle(f'{symbol} - 调整后适中布林带策略V4（正确版本）- 周K交易信号', fontsize=16, fontweight='bold')
